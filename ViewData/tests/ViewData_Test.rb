...
class ViewData_Test < MiniTest::Unit::TestCase
  ...
  def setup
    ...
    puts "hello trying to open #{modelPath}"
    ...
    if !File.exist?(sqlPath())
      ...
      var = OpenStudio::Model::OutputVariable.new('Air System Cooling Coil Total Cooling Energy', model)
      var.setReportingFrequency('Timestep')
  ...
  # the actual test
  def test_ViewData_LastOSM_AirLoop_TimeStep_Minute

    modelPathAlt = './ExampleModel_alt.osm'
    assert(File.exist?(modelPathAlt))
    assert(File.exist?(workspacePath()))
    assert(File.exist?(sqlPath()))

    vt = OpenStudio::OSVersion::VersionTranslator.new
    model = vt.loadModel(modelPathAlt)
    assert(model.is_initialized)
    model = model.get
    ...
    args_hash["variable1_name"] = 'Air System Cooling Coil Total Cooling Energy'
    args_hash["variable2_name"] = 'Zone Mean Air Temperature'
    args_hash["variable3_name"] = 'Surface Outside Face Temperature'
    ...
    current_dir = Dir.pwd
    out_dir = File.dirname(__FILE__) + "/output/LastOSM_AirLoop_TimeStep_Minute"
    if !File.exists?(out_dir)
      FileUtils.mkdir_p(out_dir)
    end
    Dir.chdir(out_dir)
